...
<div class="page-breadcrumb bg-white">
    <div class="row align-items-center">
        <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
            <h4 class="page-title">{{ __('Students Diary') }}</h4>
        </div>
        <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
            <div class="d-md-flex">
                <a href="{{ route('student-diary.create') }}"
                    class="btn btn-danger ms-auto  d-none d-md-block pull-right hidden-xs hidden-sm waves-effect waves-light text-white">{{ __('Create Student Diary') }}</a>
            </div>
        </div>
    </div>
    <!-- /.col-lg-12 -->
        ...
        <div class="col-md-12">
            ...
            <div class="white-box">
                <section class="wrapper" style="margin-top: 30px;">
                    <div class="row">
                        <div class="col-lg-12">
                            <h3 class="page-header"><i class="fa fa-laptop"></i> {{ __('Diary') }}</h3>
                        </div>
                    </div>
                    <div>
                        <x-alert />
                        <table class="table" id="diary-list-table">
                            <thead>
                                <tr>
                                    <th scope="col">{{ __('Diary Date') }}</th>
                                    <th scope="col">{{ __('Type') }}</th>
                                    <th scope="col">{{ __('The Amount') }}</th>
                                    <th scope="col">{{ __('Currency') }}</th>
                                    <th scope="col">{{ __('Bond No') }}</th>
                            ...
                            {{-- <tbody>
                                @if($diary_students)
                                @foreach ($diary_students as $diary)
                                <tr>
                                    <th scope="row">{{ $diary->diaryDate->date }}</th>
                                    <td>{{ $diary->type }}</td>
                                    <td>{{ $diary->price }}</td>
                                    <td>{{ $diary->currency_type }}</td>
                                    <td>{{ $diary->bond_no }}</td>
                                    <td>{{ $diary->student->first_name." ".$diary->student->last_name }}</td>
                                    <td>
                                        <div class="btn-group">
                                            <form method="post" action="{{route('student-diary.destroy',$diary->id)}}">
                                                @csrf
                                                @method('delete')
                                                <a class="btn btn-primary"
                                                    href="{{ route('student-diary.edit',$diary->id ) }}"><i
                ...
                @foreach ($diary_students as $diary)
                <div class="modal fade" id="modals-delete-diary{{ $diary->id }}">
                    <div class="modal-dialog">
                        <form class="create-new-nature modal-content pt-0"
                              action="{{ route('student-diary.destroy',$diary->id) }}" method="POST">
                            @csrf
                            @method('delete')
                            ...
                            <div class="modal-body flex-grow-1">
                                ...
                                <div class="form-group">
                                    <label for="username">{{__('هل تريد حذف هذا السجل')}}</label>
                                </div>
        ...
        $(document).ready(function() {
...

                        {
                            ...
                            render: function(data, type, full, meta) {
                                ...
                                return (
                                    ...
                                    'تعديل</a>' +
                                    '<a href="#" class="dropdown-item delete-record" data-toggle= "modal" data-target= "#modals-delete-diary' +
                                    id + '">' +
            ...
            $('#filter').click(function() {
                ...
                if (from_date != '' && to_date != '') {
                    $('#diary-list-table').DataTable().destroy();
