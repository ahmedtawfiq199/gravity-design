...
<div class="page-breadcrumb bg-white">
    <div class="row align-items-center">
        <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
            <h4 class="page-title">{{ __('Clients Diary') }}</h4>
        </div>
        <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
            <div class="d-md-flex">
                <a href="{{ route('client-diary.create') }}"
                    class="btn btn-danger ms-auto  d-none d-md-block pull-right hidden-xs hidden-sm waves-effect waves-light text-white">{{ __('Create Client Diary') }}</a>
            </div>
        </div>
    </div>
    <!-- /.col-lg-12 -->
                        ...
                        <table class="table" id="diary-list-table">
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col">{{ __("Statment") }}</th>
                                    <th scope="col">{{ __('Action') }}</th>
                                </tr>
                            </thead>
                            {{-- <tbody>
                                @if($diary_clients)
                                @foreach ($diary_clients as $diary)
                                <tr>
                                    ...
                                    <td>{{ $diary->bond_no }}</td>
                                    <td>{{ $diary->statment }}</td>
                                    <td>
                                        <div class="btn-group">
                                            <form method="post" action="{{route('client-diary.destroy',$diary->id)}}">
                                                @csrf
                                                @method('delete')
                                                <a class="btn btn-primary"
                                                    href="{{ route('client-diary.edit',$diary->id ) }}"><i
                ...
                <div class="modal fade" id="modals-delete-diary{{ $diary->id }}">
                    <div class="modal-dialog">
                        <form class="create-new-nature modal-content pt-0"
                              action="{{ route('client-diary.destroy',$diary->id) }}" method="POST">
                            @csrf
                            @method('delete')
